...
int arp_resolve (eth_iface_t * iface, ipv4_addr_t ip_addr, mac_addr_t mac_addr) {
      ...
      int eth_send_flag = eth_send (iface, MAC_BCAST_ADDR, ETH_ARP_TYPE,(unsigned char *) arp_request_ptr, sizeof (arp_request));

	if( eth_send_flag == -1 ) {

		//error
		return eth_send_flag; 
	}

      arp_reply_ptr = (arp_msg_ptr) malloc (ETH_MTU); //buffer
      
      int eth_recv_flag = eth_recv (iface, mac_addr, ETH_ARP_TYPE, (unsigned char*) arp_reply_ptr, (long int) 2000);

	if( eth_recv_flag == -1 ) {

		//error
		return eth_recv_flag; 
	}
...
int main (int argc, char *argv[]){
  ...
  if (argc != 3){
   ...
   /*We convert the ip address received as an argument from a string to a particular format, and then store 
